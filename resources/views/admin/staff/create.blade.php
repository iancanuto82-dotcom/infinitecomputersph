<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-900 leading-tight">Add staff account</h2>
                <p class="mt-1 text-sm text-gray-600">Create a staff login and choose what it can view or edit.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="rounded-xl bg-white shadow-sm ring-1 ring-black/10 p-6">
                <form action="{{ route('admin.staff.store') }}" method="POST" class="space-y-5">
                    ...
                    <div class="rounded-lg bg-gray-50 p-4 ring-1 ring-inset ring-black/10">
                        ...
                        @php($themePreference = old('theme_preference', 'light'))
                    ...
                    <div class="rounded-lg bg-gray-50 p-4 ring-1 ring-inset ring-black/10">
                        ...
                        @php($oldPermissions = old('permissions', []))
                                        ...
                                        @foreach ($group['permissions'] as $permissionKey => $permissionLabel)
                                            <label class="flex items-start gap-2 text-sm text-gray-900">
                                                <input type="checkbox" name="permissions[]" value="{{ $permissionKey }}"
                                                    {{ in_array($permissionKey, (array) $oldPermissions, true) ? 'checked' : '' }}
